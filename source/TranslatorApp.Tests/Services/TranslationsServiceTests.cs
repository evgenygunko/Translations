...
namespace TranslatorApp.Tests.Services
{
    ...
    public class TranslationsServiceTests
    {
        ...
        [TestMethod]
        public async Task LookUpWordInDictionaryAsync_WhenDetectedLanguageIsEnabled_ReassignsSourceLanguage()
        {
            const string searchText = "café";
            string sourceLanguage = SourceLanguage.Danish.ToString();
            IReadOnlyList<string> activeDictionaries = [SourceLanguage.Spanish.ToString()];
        ...
        [TestMethod]
        public async Task LookUpWordInDictionaryAsync_WhenDetectedLanguageIsNotEnabled_KeepsOriginalSourceLanguage()
        {
            const string searchText = "café";
            string sourceLanguage = SourceLanguage.Danish.ToString();
            IReadOnlyList<string> activeDictionaries = [SourceLanguage.Danish.ToString()];

            WordModel wordModel = _fixture.Create<WordModel>() with { SourceLanguage = SourceLanguage.Danish };

            var lookUpWordMock = _fixture.Freeze<Mock<ILookUpWord>>();
            lookUpWordMock.Setup(x => x.LookUpWordAsync(searchText, SourceLanguage.Danish.ToString(), It.IsAny<CancellationToken>())).ReturnsAsync(wordModel);
        ...
        [TestMethod]
        public async Task LookUpWordInDictionaryAsync_WhenFallbackDictionaryIsNotEnabled_DoesNotTryAnotherDictionary()
        {
            const string searchText = "word to translate";
            string sourceLanguage = SourceLanguage.Danish.ToString();
            IReadOnlyList<string> activeDictionaries = [SourceLanguage.Danish.ToString()];
        ...
        [TestMethod]
        public async Task LookUpWordInDictionaryAsync_WhenFallbackDictionaryIsEnabledWithDifferentCase_TriesAnotherDictionary()
        {
            const string searchText = "word to translate";
            string sourceLanguage = SourceLanguage.Danish.ToString();
        ...
        public async Task LookUpWordInDictionaryAsync_WhenWordHasRussianSymbols_ReturnsNullWithoutCallingLookup()
        {
            ...
            string sourceLanguage = SourceLanguage.Danish.ToString();
            IReadOnlyList<string> activeDictionaries = [SourceLanguage.Danish.ToString(), SourceLanguage.Spanish.ToString()];
        ...
        #region Tests for GetAISuggestedWordsAsync

        [TestMethod]
        public async Task GetAISuggestedWordsAsync_WhenLDFlagIsTrue_UsesChatCompletionService()
        {
            ...
            openAITranslationServiceMock
                .Setup(x => x.GetTranslationSuggestionsAsync("привет", "Russian", "Danish", It.IsAny<CancellationToken>()))
                .ReturnsAsync(["hej"]);
            var sut = _fixture.Create<TranslationsService>();

            IEnumerable<string> result = await sut.GetAISuggestedWordsAsync("привет", "Danish");

            result.Should().Equal("hej");
        ...
        [TestMethod]
        public async Task GetAISuggestedWordsAsync_WhenLDFlagIsFalse_UsesResponsesService()
        {
            var launchDarklyServiceMock = _fixture.Freeze<Mock<ILaunchDarklyService>>();
            ...
            openAITranslationService2Mock
                .Setup(x => x.GetTranslationSuggestionsAsync("привет", "Russian", "Spanish", It.IsAny<CancellationToken>()))
                .ReturnsAsync(["hola"]);
            var sut = _fixture.Create<TranslationsService>();

            IEnumerable<string> result = await sut.GetAISuggestedWordsAsync("привет", "Spanish");

            result.Should().Equal("hola");
        ...
        [DataRow("ёж")]
        [DataRow("Москва")]
        public void CheckLanguageSpecificCharacters_WhenTextContainsRussianCharacters_ReturnsTrueAndRussian(string text)
        {
            var sut = _fixture.Create<TranslationsService>();
        ...
        public void CreateTranslationInputFromWordModel_Should_ReturnTranslationInput()
        {
            ...
            TranslatorApp.Models.Translation.DefinitionInput inputDefinition = result.Definition;
        ...
        public void CreateWordModelFromTranslationOutput_WhenContextIsNotReturned_UsesContextFromOriginalModel()
        {
            ...
            var translationOutput = new TranslatorApp.Models.Translation.TranslationOutput(
                Definition: new TranslatorApp.Models.Translation.DefinitionOutput(
